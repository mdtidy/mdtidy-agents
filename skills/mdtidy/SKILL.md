...
file documents under a folder, `create_folder` (idempotent — find-or-create, so
re-calling with the same name never duplicates), then pass its id as `folder_id`
to `save_file` / `update_file`.
